...
| 🟡 | Include LoRAs from main prompt (+ optional trigger extraction) | LoRAs in the main txt2img/img2img prompt are not auto-applied to the inpaint pass. There is also no way to surface the *trigger words* embedded in a LoRA filename. | Two paired checkboxes: `Use LoRAs from main prompt` (parent, `ad_use_main_loras`) scrapes `<lora:name:weight>` tags out of the main prompt and merges them into the inpaint prompt. `Append LoRA triggers from name` (child, `ad_use_lora_triggers`) parses the parenthesised substring inside a LoRA name — convention `<lora:name (trigger phrase):weight>` from [Anzhc/aadetailer-reforge](https://github.com/Anzhc/aadetailer-reforge) — and appends the trigger phrase to the prompt. The child only runs when the parent is on. Helpers: `_LORA_TAG_RE`, `_LORA_TRIGGER_RE`, `_extract_lora_tags`, `_extract_lora_triggers`, `_merge_lora_tags`, `_append_lora_triggers`.<br>_Trigger-extraction sub-feature added 2026-05-16, both sub-features still being tested by the repo owner._ |
...
| 🟡 | Persistent last-used settings | Tab values revert to their static defaults at WebUI restart unless you manually re-apply a "set defaults" through Settings. | Every Generate stashes per-tab widget state to `<extension_root>/user_state.json` (atomic write). State is **scoped by pipeline** — `"txt2img:<tab>"` and `"img2img:<tab>"` keys are independent, so changing settings in one mode never affects the other. Restored as initial values at the next WebUI start. Gated by `Settings → ADetailer → Remember last used settings` (default on). Implemented in `adetailer/persistence.py`. |
...
| 🟡 | Apply only on hires.fix | ADetailer always runs on every `postprocess_image` call, including the lowres txt2img call that precedes the hires.fix upscale — wasted compute since the hires step overwrites the lowres detail. | **Txt2img-only** per-tab checkbox (`ad_apply_on_hires_only`, hidden in img2img) that skips the tab's pass during the lowres pre-hires call and only runs ADetailer once the hires.fix upscaled image is ready. When hires.fix is off in txt2img, the tab is skipped entirely (the user explicitly asked for "hires only"). The toggle is a no-op in img2img regardless of how it got set (e.g. via a preset shared from txt2img). Decision matrix in helper `_should_skip_for_hires_only(p, args)`. Inspired by [Anzhc/aadetailer-reforge](https://github.com/Anzhc/aadetailer-reforge). |
| 🟡 | Bounding-box mask (segmentation models) | When a segmentation YOLO model produces a per-pixel mask, ADetailer uses that mask verbatim — sometimes too tight against the subject, leaving little context for the inpaint to blend naturally. | Per-tab checkbox (`ad_use_bbox_mask`) in the **Mask preprocessing** accordion. Forces the rectangular bounding box as the inpaint mask even when the detector provided a precise per-pixel segmentation mask. No effect on bbox-only detectors (they were already using bboxes). Implemented as a single conditional in `ultralytics_predict`. Inspired by [newtextdoc1111/adetailer](https://github.com/newtextdoc1111/adetailer). |
